...
{
    // TODO Implemented // Test Complete.

	unsigned int seek = lseek(fd, id*STUDENT_RECORD_SIZE, SEEK_SET);
    	if(seek == (off_t) -1) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    struct student temp_record_buff;
    int status = get_student(fd, id, &temp_record_buff);
    switch (status) {
	    case ERR_DB_FILE:
		    return ERR_DB_FILE;
	    case NO_ERROR:
		    if(!(memcmp(&temp_record_buff, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0)) {
	    ...
	    case SRCH_NOT_FOUND: 
		    struct student new_student_entry;
		    memset(&new_student_entry, 0, STUDENT_RECORD_SIZE);
		    new_student_entry.id = id;
		    new_student_entry.gpa = gpa;
	 	    unsigned int i;
		    for (i = 0; i < (sizeof(new_student_entry.fname) - 1) && (!(fname[i] == '\0')); i++) {
			    
			    new_student_entry.fname[i] = fname[i];

		    } new_student_entry.fname[i] = '\0';

		    for (i = 0; i < (sizeof(new_student_entry.lname) - 1) && (!(lname[i] == '\0')); i++) {
			    
			    new_student_entry.lname[i] = lname[i];

		    } new_student_entry.lname[i] = '\0';
		  ...
		  int bytes_written = write(fd, &new_student_entry, STUDENT_RECORD_SIZE);
    ...
    switch (status) {
	    ...
	    case NO_ERROR:
			...
			return ERR_DB_OP;
			    ...
			    memset(&deleted_student, 0, STUDENT_RECORD_SIZE);
			    ...
			    int bytes_written = write(fd, &deleted_student, STUDENT_RECORD_SIZE);
...
int count_db_records(int fd)
{
    ...
    while((bytes_read = read(fd, &temp_record_buff, STUDENT_RECORD_SIZE)) > 0) {
    ...
    } else printf(M_DB_RECORD_CNT, count);
...
{
    // TODO Implemented // Test Complete.
     unsigned int seek = lseek(fd, 0, SEEK_SET);
    ...
    while((bytes_read = read(fd, &temp_record_buff, STUDENT_RECORD_SIZE)) > 0) {
	    ...
	    if (!(memcmp(&temp_record_buff, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0)) {
		    ...
		    printf(STUDENT_PRINT_FMT_STRING, temp_record_buff.id, temp_record_buff.fname, temp_record_buff.lname, (float) (temp_record_buff.gpa)/100);
...
{
    // TODO Impleneted TODO Test Complete.
